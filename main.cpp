...
#define PN 50//可保存的点的数目 
#define LN 50//可保存线的数目 
#define RN 50//可保存三角形的数目 
#define PI 3.1415926
...
void addLine() {
    ...
    cout<<"起点坐标:"<<v1.transpose()<<endl;
    cout<<"终点坐标:"<<v2.transpose()<<endl; 
}
    ...
    for(int i=0;i<PN;i++) {
        if(point[i].getName()==s) {
            ...
            Vector2d v1 = point[i].getPoint();
             v1=v1+v;
             cout<<s<<"平移后的点的坐标:"<<v1.transpose()<<endl;    
        } 
    }
    ...
    for(int i=0;i<LN;i++) {
        if(line[i].getName()==s) {
            ...
            Vector2d v2 = line[i].getEndPoint();
             v1 = v1+v;
             v2 = v2+v; 
             cout<<"直线"<<s<<"平移后起点的坐标:"<<v1.transpose()<<endl;
             cout<<"直线"<<s<<"平移后终点的坐标:"<<v2.transpose()<<endl;    
        } 
    }
    ...
    for(int i=0;i<RN;i++) {
        if(rect[i].getName()==s) {
            ...
            Vector2d v3 = rect[i].getPoint3();
             ...
             v2 = v2+v; 
             v3 = v3+v;
             cout<<"三角形"<<s<<"点1平移后的坐标:"<<v1.transpose()<<endl;
             cout<<"三角形"<<s<<"点2平移后的坐标:"<<v2.transpose()<<endl;
             cout<<"三角形"<<s<<"点3平移后的坐标:"<<v2.transpose()<<endl;    
        } 
    }
    
    if(find==false) {
        cout<<"没有找到对应的点线面"<<endl; 
    }
}
    ...
    for(int i=0;i<LN;i++) {
        if(line[i].getName()==s) {
            ...
            Vector2d v2 = line[i].getEndPoint();
             v1 = rot*v1;
             v2 = rot*v2; 
             cout<<"直线"<<s<<"旋转后起点的坐标:"<<v1.transpose()<<endl;
             cout<<"直线"<<s<<"旋转后终点的坐标:"<<v2.transpose()<<endl;    
        } 
    }    
        ...
        if(rect[i].getName()==s) {
            ...
            Vector2d v3 = rect[i].getPoint3();
             ...
             v2 = rot*v2; 
             v3 = rot*v3;
             cout<<"三角形"<<s<<"点1旋转后的坐标:"<<v1.transpose()<<endl;
             cout<<"三角形"<<s<<"点2旋转后的坐标:"<<v2.transpose()<<endl;
             cout<<"三角形"<<s<<"点3旋转后的坐标:"<<v2.transpose()<<endl;    
        } 
    }
